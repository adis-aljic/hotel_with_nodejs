...
app.get('/guest/:username', function (req, res) {
  ...
  db.query(recieptSQL, function (err, data1) {
    if (err) throw err;
      ...
      let reciept = data1[0];
      res.render('guest', {
        first_name: reciept.first_name,
        room_number: reciept.room_number,
        last_name: reciept.last_name,
        username: reciept.username,
        password: reciept.password,
        check_in_date: reciept.check_in_date.toISOString().slice(0, 10),
        check_out_date: reciept.check_out_date.toISOString().slice(0, 10),
        total_price_for_room: reciept.total_price_for_room,
        total_price_sauna: reciept.total_price_sauna,
        total_price_restaurant: reciept.total_price_restaurant,
        total_price_cinema: reciept.total_price_cinema,
        total_price_gym: reciept.total_price_gym,
        total_price_pool: reciept.total_price_pool,
        total_price_for_booking: reciept.total_price_for_booking,
        reciept_status: reciept.reciept_status,
      });
    }
  });
});
...
app.post('/adminguest', urlencodedParser, function (req, res) {
  ...
  if (info.check_in_date < info.check_out_date) {
    ...
    // console.log(hash);
    var newguest = new guestModule.newguestClass(
      info.first_name,
      info.last_name,
      info.date_of_birth,
      info.country,
      ...
      info.email,
      info.gender,
      info.prefered_language,
      info.username,
      info.password,
      info.document_for_indefication,
      info.number_of_document_for_indefication
    );
    guestModule.addGuestSQL(
      newguest,
      newguest.first_name,
      newguest.last_name,
      newguest.date_of_birth,
      newguest.gender,
      newguest.country,
      newguest.city,
      newguest.prefered_language,
      newguest.phone_number,
      newguest.email,
      newguest.document_for_indefication,
      newguest.document_for_indefication +
        ' ' +
        newguest.number_of_document_for_indefication,
      newguest.username,
      newguest.password
    );
    ...
    roomModule.addGuestToRoom(
      guestRoom.room_number,
      guestRoom.room_status,
      guestRoom,
      newguest.username
    );

    // creating booking for guest
    var guestBooking = new bookingModule.guestBookingClass(
      info.check_in_date,
      info.check_out_date,
      info.price_per_night
    );
    bookingModule.createBooking(
      guestBooking,
      guestBooking.check_in_date,
      guestBooking.check_out_date,
      guestBooking.price_per_night,
      guestRoom.room_number,
      newguest.username
    );
    ...
    if (
      info.date_to_sauna != '' &&
      info.date_from_sauna != '' &&
      info.date_to_sauna > info.date_from_sauna
    ) {
      var guestSauna = new saunaModule.guestSaunaClass(
        info.date_from_sauna,
        info.date_to_sauna
      );
      saunaModule.addSaunaToGuest(
        guestSauna,
        guestSauna.date_from_sauna,
        guestSauna.date_to_sauna,
        guestSauna.price_per_day_sauna,
        guestRoom.room_number,
        newguest.username
      );
    } else {
    ...
    if (
      info.date_to_restaurant != '' &&
      info.date_from_restaurant != '' &&
      info.date_to_restaurant > info.date_from_restaurant
    ) {
      var guestRestaurant = new restaurantModule.guestRestaurantClass(
        info.date_from_restaurant,
        info.date_to_restaurant
      );
      restaurantModule.addRestaurantToGuest(
        guestRestaurant,
        guestRestaurant.date_from_restaurant,
        guestRestaurant.date_to_restaurant,
        guestRestaurant.price_per_day_restaurant,
        guestRoom.room_number,
        newguest.username
      );
    } else {
    ...
    if (
      info.date_to_pool != '' &&
      info.date_from_pool != '' &&
      info.date_to_pool > info.date_from_pool
    ) {
      var guestPool = new poolModule.guestPoolClass(
        info.date_from_pool,
        info.date_to_pool
      );
      poolModule.addPoolToGuest(
        guestPool,
        guestPool.date_from_pool,
        guestPool.date_to_pool,
        guestPool.price_per_day_pool,
        guestRoom.room_number,
        newguest.username
      );
    } else {
    ...
    if (
      info.date_to_cinema != '' &&
      info.date_from_cinema != '' &&
      info.date_to_cinema > info.date_from_cinema
    ) {
      var guestCinema = new cinemaModule.guestCinemaClass(
        info.date_from_cinema,
        info.date_to_cinema
      );
      cinemaModule.addCinemaToGuest(
        guestCinema,
        guestCinema.date_from_cinema,
        guestCinema.date_to_cinema,
        guestCinema.price_per_day_cinema,
        guestRoom.room_number,
        newguest.username
      );
    } else {
    ...
    if (
      info.date_to_gym != '' &&
      info.date_from_gym != '' &&
      info.date_to_gym > info.date_from_gym
    ) {
      var guestGym = new gymModule.guestGymClass(
        info.date_from_gym,
        info.date_to_gym
      );
      gymModule.addGymToGuest(
        guestGym,
        guestGym.date_from_gym,
        guestGym.date_to_gym,
        guestGym.price_per_day_gym,
        guestRoom.room_number,
        newguest.username
      );
    } else {
...
app.post('/adminemployee', urlencodedParser, function (req, res) {
  ...
  var sql = `INSERT INTO employees (first_name, last_name, job_title, date_of_birth,country,city,phone_number,email,
        ...
        VALUES("${infoAdmin.first_name}","${infoAdmin.last_name}","${
    infoAdmin.job_title
  }","${infoAdmin.date_of_birth}",
        "${infoAdmin.country}","${infoAdmin.city}","${
    infoAdmin.phone_number
  }","${infoAdmin.email}","${infoAdmin.gender}",
        "${infoAdmin.languages.join(',')}","${
    infoAdmin.document_for_indefication
  }","${infoAdmin.document_for_indefication} ${
    infoAdmin.number_of_document_for_indefication
  }",
...
app.post('/adminlogin', urlencodedParser, function (req, res) {
  ...
  user_passModul.checkEmployee(
    res,
    req.body.username_employees,
    req.body.password_employees
  );
});
    ...
    if (err) {
      ...
      res.render(`findbooked`, {
        ...
        number_of_document_for_indefication:
          book.number_of_document_for_indefication.slice(
            book.number_of_document_for_indefication.lastIndexOf(' ')
          ),
...
app.post('/findbooked', urlencodedParser, function (req, res) {
  ...
  if (early_checkout != '') {
    ...
    db.query(sql1, function (err, data) {
      if (err) throw err;
        ...
        db.query(
          `UPDATE reciept SET reciept_status = "paid" WHERE username = "${username}";
                    UPDATE booking SET check_out_date = "${new Date()
                      .toISOString()
                      .slice(0, 10)}"`,
          function (err, data) {
  ...
  if (
    early_checkout_sauna != '' ||
    early_checkout_pool != '' ||
    early_checkout_gym != '' ||
    early_checkout_cinema != '' ||
    early_checkout_restaurant != ''
  ) {
    db.query(sql1, function (err, data) {
      if (err) throw err;
      else {
        console.log(
          `Guest with username ${username} has changed aditional services`
        );
        res.render('./findbooking');
      }
    });
